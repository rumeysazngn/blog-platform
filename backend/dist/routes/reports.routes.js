...
router.get("/", auth_1.authenticate, async (req, res) => {
    try {
        ...
        const reports = await prisma_1.default.raporlar.findMany({
...
router.put("/:id", auth_1.authenticate, async (req, res) => {
    try {
        ...
        const updated = await prisma_1.default.raporlar.update({
...
------------------------------------------*/
router.delete("/delete-target/:reportId", auth_1.authenticate, async (req, res) => {
    try {
        if (req.user?.role !== "admin") {
            return res.status(403).json({ success: false, message: "Yetkiniz yok" });
        }
        const reportId = Number(req.params.reportId);
        const report = await prisma_1.default.raporlar.findUnique({
            where: { rapor_id: reportId },
        });
        if (!report) {
            return res
        ...
        await prisma_1.default.raporlar.update({
        ...
        if (report.hedef_tur === "yazi") {
            ...
            await prisma_1.default.yorum.delete({
...
------------------------------------------*/
router.post("/post/:postId", auth_1.authenticate, async (req, res) => {
    try {
        ...
        const exists = await prisma_1.default.yazi.findUnique({ where: { yazi_id: postId } });
        if (!exists)
            return res.status(404).json({ success: false, message: "Yazı bulunamadı" });
        const rapor = await prisma_1.default.raporlar.create({
            data: {
...
------------------------------------------*/
router.post("/comment/:commentId", auth_1.authenticate, async (req, res) => {
    try {
