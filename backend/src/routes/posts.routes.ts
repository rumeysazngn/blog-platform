...
interface AIContentBasedResponse {
  target_post_id: number;
  recommended_post_ids: number[];
}

interface AICollaborativeResponse {
  recommended_post_ids: number[];
}
...
async function analyzeText(text: string): Promise<string> {
  try {
    const res = await axios.post<{ label: string }>(`${AI_URL}/analyze`, { text });
...
------------------------------------------------------------- */
      ...
      ------------------------------------------------------------- */
      const baseSlug = slugify(baslik, { lower: true, strict: true });
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        let kapak_resmi: string | null = null;
        if (req.file) kapak_resmi = `/uploads/${req.file.filename}`;
...
------------------------------------------------------------- */
router.get("/recommend/user/:userId", authenticate, async (req, res) => {
  try {
    ...
    const likes = await prisma.begeni.findMany({
    ...
    const result = (await response.json()) as AICollaborativeResponse;
...
router.get("/:postId/recommend", async (req, res) => {
  try {
    ...
    const result = (await response.json()) as AIContentBasedResponse;
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const existing = await prisma.yazi.findUnique({
...
router.delete("/:id", authenticate, async (req, res) => {
  try {
    ...
    const post = await prisma.yazi.findUnique({ where: { yazi_id: postId } });
    ...
    await prisma.yazi_kategorileri.deleteMany({
      where: { yazi_id: postId },
    });

    await prisma.yazi.delete({ where: { yazi_id: postId } });
